...
if st.session_state.user_connected is True:
    with st.sidebar:
        with st.container(border=True):
            st.markdown(f"**{user_manager.get_any_user_data(user_id=st.session_state.user_id, column="xp")}🔥**")
            st.markdown(f"**{user_manager.get_any_user_data(user_id=st.session_state.user_id, column="credits")}⭐**")
